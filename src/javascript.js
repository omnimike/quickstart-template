...
function assignment(stmnt) {
    return comment(stmnt.comment) + 'const ' + camelCase(stmnt.variable) +
...
function sdkInit(args) {
    ...
    let out = 'const ' + sdkName + ' = new Learnosity();\n';
    out += 'const ' + initOptsName + ' = ' + sdkName + '.init(\n';
    let argsStr = '';
    if (remainingArgs[0]) {
